...
def conectar_db():
    try:
        ...
        st.error(f"⚠️ Error de conexión con la base de datos. Verifique la configuración de acceso.")
        return None

# ==========================================
# 2. ESTILOS CSS (DISEÑO TITANIUM PROFESIONAL)
# ==========================================
st.markdown("""
<style>
...
# ==========================================
# 3. DICCIONARIO GIGANTE DE ACTIVOS (Reconstruido)
# ==========================================
tickers_nombres = {
        ...
        with st.spinner(f"Sincronizando datos de {simbolo_real}..."):
            try:
                ...
                if not datos.empty:
                    ...
                    # SOLUCIÓN BME: Extraer el último precio no nulo
                    ...
                    if sector and industria: st.caption(f"🏢 **Sector:** {sector} | **Industria:** {industria}")
                    if resumen_largo:
...
# ------------------------------------------
# PESTAÑA 2: RADAR DE CAZA CON AUTO-GUARDADO (STRICT MODE BASE 40)
# ------------------------------------------
with tab2:
    ...
    if mercado_objetivo:
        ...
        try:
            ...
            if len(spy_hist) >= 21: alphaSPY = ((spy_hist['Close'].iloc[-1] / spy_hist['Close'].iloc[-21]) - 1) * 100
...
with tab3:
    ...
    st.write("Auditoría de rendimiento. Las acciones que alcanzan o superan los 90 puntos en el Radar se guardan aquí de forma permanente.")
    
    ws = conectar_db()
    if ws:
        data_sheet = ws.get_all_records()
        if not data_sheet:
            st.info("La base de datos está vacía. Ejecute el escáner para registrar activos.")
        else:
            ...
            if st.button("🔄 Ejecutar Auditoría de Rendimiento", use_container_width=True):
                with st.spinner("Sincronizando precios en tiempo real..."):
                    ...
                    if tot > 0:
                        ...
                        m1.metric("Win Rate", f"{(len(exitos)/tot)*100:.1f}%", help="Tasa de éxito de las señales institucionales.")
                        m2.metric("Alpha Medio", f"{alpha_total/tot:+.2f}%", help="Rentabilidad media desde la detección.")
